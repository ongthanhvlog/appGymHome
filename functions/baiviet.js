...
async function xuLyCapNhatBaiVietMoi() {
    ...
    for (const topic of topics) {
        try {
            const res = await axios.get(topic.url, { headers: headers, timeout: 15000 });
            ...
            for (const baiViet of unique.slice(0, 4)) {
                try {
                    ...
                    const detailRes = await axios.get(baiViet.url, { headers: headers, timeout: 15000 });
...
async function xuLyLinkBaiViet(baiVietUrl, tag) {
    ...
    if (existing.exists) return { success: false, reason: "existed", docId };

    const detailRes = await axios.get(baiVietUrl, { headers: headers, timeout: 15000 });
    const data = tachNoiDungBaiViet(detailRes.data);
    const $bm = cheerio.load(detailRes.data);
...
async function xoaBaiVietCu() {
    ...
    const snapshot = await db.collection("BaiViet").where("ngayDang", "<", thoiDiemHetHan).get();
...
exports.tuDongXoaBaiViet = onSchedule("0 0 * * *", async (event) => { await xoaBaiVietCu(); });
